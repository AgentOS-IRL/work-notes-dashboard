...
- `list_notes` lists note titles for discovery and update selection
- `list_notes` returns `{ "titles": ["..."] }` instead of full note objects
- `list_notes` defaults to `{ "limit": 10, "offset": 0 }`
- `list_notes` accepts paging arguments like `{ "limit": 10, "offset": 20 }` to fetch later pages of titles
